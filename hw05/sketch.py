...
def pollAccel():
    ...
    init1.write('adxl345 0x53')#this is the hardcoded address inherent in the chip
    sensor=open('/sys/class/i2c-adapter/i2c-2/2-0053/iio:device1/in_accel_z_raw','r')#read the z axis
    while True:
        sensor.seek(0)#go to sof to get new value
        temp=sensor.readline()
        #print(temp)
        if '-' in temp: #look for negative
            clearScreen()
        time.sleep(.5)
...
if __name__=="__main__": #if this is being run directly and not used as a library
    ...
    print(board)
    t1=threading.Thread(target=pollAccel)
    t1.start()#start up polling loop in parallel thread
