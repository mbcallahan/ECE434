/**
 * @file   led.c
 * @author Derek Molloy, edited Matthew Callahan
 * @date   19 April 2015
...
MODULE_LICENSE("GPL");
MODULE_AUTHOR("Derek Molloy, Matthew Callahan");
...
static unsigned int blinkPeriod1 = 1400;
 ...
 * Changed to 660 on advice of Dr. Yoder
 */
...
/** @brief The LED Flasher secondary kthread loop
 *
 *  @param arg A void pointer used in order to pass data to the thread
 *  @return returns 0 if successful
 */
static int flash1(void *arg){
   printk(KERN_INFO "EBB LED: Second Thread has started running \n");
   while(!kthread_should_stop()){           // Returns true when kthread_stop() is called
      set_current_state(TASK_RUNNING);
   ...
   printk(KERN_INFO "EBB LED: Second Thread has run to completion \n");
   return 0;
}
...
static int __init ebbLED_init(void){
   ...
   ledOn1 = true;
   gpio_request(gpioLED1, "sysfs");          // gpioLED is 48 by default, request it
   gpio_direction_output(gpioLED1, ledOn1);   // Set the gpio to be in output mode and turn on
   gpio_export(gpioLED1, false);  // causes gpio48 to appear in /sys/class/gpio
                                 // the second argument prevents the direction from being changed
   ...
   task1 = kthread_run(flash1,NULL, "LED_flash_thread1");
